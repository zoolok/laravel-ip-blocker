...
Опции:
- `--all` — разблокировать все IP и очистить все подозрительные запросы
- `--force` — пропустить подтверждение
- `--no-nginx` — не перегенерировать deny-конфиг
...
'exclude_paths' => [
    '/healthcheck',
    '/admin*',                // админ-панель
    '/vendor/moonshine*',     // ассеты админ-панели
    '/lk*',                   // личный кабинет
    '/storage*',
],
```
...
- **Не занижайте пороги.** Для реальных пользователей `IP_BLOCKER_MAX_REQUESTS`
  ниже ~100 и `IP_BLOCKER_MAX_UNIQUE_URLS` ниже ~20 дают ложные блокировки
  активных сессий. Порог `limit: 5` подходит только для тестов.
